...
export class PaynowService {
  ...
  static async initiateTransaction(params: PaynowInitiateParams): Promise<PaynowInitiateResult> {
    ...
    try {
      // 1. Try server-side proxy route first (Vercel Serverless / Vite dev middleware)
      ...
      try {
        const response = await fetch(endpoint, {
          ...
          body: JSON.stringify({
            ...
            integrationId: config.integrationId || undefined,
            integrationKey: config.integrationKey || undefined
          })
        });
        ...
        console.warn('[PAYNOW] Serverless route unreachable, checking client credentials fallback:', networkError);
      }

      // 2. Direct browser fallback if local client credentials exist
      if (config.integrationId && config.integrationKey) {
        ...
        const isMobileExpress = Boolean(params.phone && (params.paymentMethod === 'EcoCash' || params.paymentMethod === 'OneMoney'));
        if (isMobileExpress && params.phone) {
          payload.set('phone', params.phone.trim());
          payload.set('method', params.paymentMethod?.toLowerCase() === 'onemoney' ? 'onemoney' : 'ecocash');
        }

        const directEndpoint = isMobileExpress
        ...
        if (directResponse.ok) {
          ...
          if (respStatus?.toLowerCase() === 'ok') {
            return {
              success: true,
              reference,
              browserUrl: responseParams.get('browserurl') || undefined,
              pollUrl: responseParams.get('pollurl') || undefined,
            ...
            return {
              success: false,
              reference,
              error: responseParams.get('error') || 'Paynow could not authorize the transaction.'
            };
          }
        }
      }
      ...
      return {
        success: false,
        reference,
        error: 'Paynow gateway is not configured. Please add PAYNOW_INTEGRATION_ID and PAYNOW_INTEGRATION_KEY in environment variables or Settings.'
      };
    } catch (e: any) {
